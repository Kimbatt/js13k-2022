...
export function MetalTexture(c: ReturnType<typeof CreateWebglCanvas>, w: number, h: number,
    scale = 5,
...
vec4 getColor(vec2 uv)
{
    ...
    // voro = valueRamp2(0.4, 0.3, 0.6, 0.7, voro);

    ${isAlbedo
            ? `
        vec4 albedo = colorRamp2(vec4(0.0, 0.0, 0.0, 1), 0.2, vec4(1.0, 1.0, 1.0, 1), 0.8, voro) * vec4(${baseColor.join(",")}, 1);
    ...
    c.DrawWithShader([ShaderUtils, FBM, Voronoi, shader(true), edgeBlend("getColor")], mainImage, w, h, [], albedo);

    const heightMap = c.CreateTexture(w, h);
    c.DrawWithShader([ShaderUtils, FBM, Voronoi, shader(false), edgeBlend("getColor")], mainImage, w, h, [], heightMap);
