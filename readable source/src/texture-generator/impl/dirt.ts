...
export function DirtTexture(c: ReturnType<typeof CreateWebglCanvas>, w: number, h: number,
    scale = 5,
...
vec4 getColor(vec2 uv)
{
    ...
    ${isAlbedo
            ? `
        vec4 albedo = colorRamp2(vec4(0, 0, 0, 1), 0.08, vec4(${baseColor.join(",")}, 1), 0.67, noise3);
    ...
    c.DrawWithShader([ShaderUtils, FBM, VoronoiDistance, shader(true), edgeBlend("getColor")], mainImage, w, h, [], albedo);

    const heightMap = c.CreateTexture(w, h);
    c.DrawWithShader([ShaderUtils, FBM, VoronoiDistance, shader(false), edgeBlend("getColor")], mainImage, w, h, [], heightMap);
